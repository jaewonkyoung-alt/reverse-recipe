...
| 액터 | 일반 사용자 |
| 사전 조건 | 로그인 상태 |
| 기본 흐름 | 재료명·카테고리 입력 → 117개 내장 DB에서 소비기한 자동 계산 → DB 저장 → 긴급도(빨강/노랑/초록) 표시 |
...
|  Pages: Home / Fridge / Recipe / Shopping / MyRipe          |
...
|                                                             |
|  /auth   /ingredients   /recipes   /shopping               |
|                    |                                        |
|       ingredientService / recipeService                     |
|                    |  (GEMINI_API_KEY 있을 때)               |
|                    v                                        |
|          Google Gemini 2.5 Flash API                        |
|          (없으면 Mock 레시피 10종 fallback)                   |
+-----------------------------+--------------------------------+
                              | SQLite (better-sqlite3)
                              v
+--------------------------------------------------------------+
|  reverse_recipe.db  (파일 기반, 별도 DB 서버 불필요)           |
|  users / ingredients / recipe_history / shopping_list /     |
|  green_points / recipe_cache                                |
+--------------------------------------------------------------+
```
...
| 메인 (로그인 전) | 로그인 화면 | 홈 (로그인 후) |
|:-:|:-:|:-:|
| ![메인 화면](images/01_home.png) | ![로그인](images/02_login.png) | ![홈 로그인 후](images/03_home_loggedin.png) |

**냉장고 관리**

| 재료 목록 | 재료 추가 |
|:-:|:-:|
| ![냉장고 재료 목록](images/04_fridge.png) | ![재료 추가 모달](images/05_add_ingredient.png) |

**AI 레시피 추천**

| 추천 목록 | 레시피 상세 |
|:-:|:-:|
| ![레시피 추천 목록](images/06_recipe_list.png) | ![레시피 상세](images/07_recipe_detail.png) |

**마이리피 / 그린포인트**

| 마이리피 |
|:-:|
| ![마이리피](images/08_myripe.png) |
...
현재 Google Gemini 무료 API를 사용하고 있어 레시피 추천에 약 15~30초가 소요된다. 무료 요청 한도 초과 시 서비스가 중단될 수 있으며, 향후 유료 플랜 전환 또는 자체 추천 알고리즘 병행이 필요하다.

**재료 입력 자동화 미구현**

현재 재료를 사용자가 직접 수동 입력해야 한다. 영수증 사진 또는 냉장고 사진을 OCR로 인식해 자동 등록하는 기능을 구현하지 못했으며, 이는 사용자 편의성의 핵심 개선 과제로 남아있다.

**인증 시스템 미완성**

소셜 로그인(카카오·구글 OAuth) 및 비밀번호 찾기 기능을 구현하지 못했다. 현재는 이메일+비밀번호 방식만 지원하여 실사용 환경에서의 접근성이 낮다.

**보안 취약점 및 강화 미완료**

현재 JWT 단일 토큰 방식만 적용되어 있으며, 2단계 인증(2FA) 및 RefreshToken 갱신 로직이 미구현 상태이다. 또한 API 요청에 대한 입력값 검증과 SQL 인젝션 방어 처리가 일부 미흡하여, 실서비스 배포 전 보안 강화가 필요하다.
